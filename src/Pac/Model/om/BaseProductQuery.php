 ...
 *
 * @method ProductQuery leftJoinPurchase($relationAlias = null) Adds a LEFT JOIN clause to the query using the Purchase relation
 * @method ProductQuery rightJoinPurchase($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Purchase relation
 * @method ProductQuery innerJoinPurchase($relationAlias = null) Adds a INNER JOIN clause to the query using the Purchase relation
 *
 ...
 * @method Product findOneByVersion(string $version) Return the first Product filtered by the version column
 ...
 * @method array findByVersion(string $version) Return Product objects filtered by the version column
...
abstract class BaseProductQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `name`, `version`, `created_at`, `updated_at` FROM `product` WHERE `id` = :p0';
        try {
    ...
    public function filterByPurchase($purchase, $comparison = null)
    {
        if ($purchase instanceof Purchase) {
            return $this
                ->addUsingAlias(ProductPeer::ID, $purchase->getProductId(), $comparison);
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   \Pac\Model\PurchaseQuery A secondary query class using the current class as primary query
     */
    public function usePurchaseQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinPurchase($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'Purchase', '\Pac\Model\PurchaseQuery');
    }
